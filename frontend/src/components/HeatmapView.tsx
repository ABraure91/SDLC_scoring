...
export default function HeatmapView(props: {
  ...
  const { toolNames, stepNames, matrix, minScore, maxScore, filters } = props

  const [selectedStep, setSelectedStep] = useState<string | null>(null)
  const plotRef = useRef<HTMLDivElement>(null)

  // Réinitialiser la sélection si l'étape sélectionnée n'est plus dans les filtres
  useEffect(() => {
    if (selectedStep && !filters.selectedSteps.includes(selectedStep)) {
      setSelectedStep(null)
    }
  }, [selectedStep, filters.selectedSteps])
